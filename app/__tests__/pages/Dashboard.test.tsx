...
import { mockUserContext, getUserContext } from "../../__test-fixtures__/userContextFixtures";
...
describe("when user has a connected wallet", () => {
  ...
  it("should have a disconnect button", async () => {
    ...
    await waitFor(() => {
      expect(mockUserContext.handleConnection).toBeCalledTimes(1);
    });
  });
});
...
describe("when user has no passport", () => {
  ...
  const mockUserContextWithNoPassport = getUserContext({ passport: undefined });
  ...
  it("should display a loading spinner, and call create passport", async () => {
    ...
    await waitFor(() => {
      expect(mockUserContext.handleCreatePassport).toBeCalledTimes(1);
    });
  });
});
